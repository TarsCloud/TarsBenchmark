<template>
  <div class="benchmark_wrapper">
    ...
    <let-table
      :data="fnList"
      :title="$t('inf.benchmark.fnlist')"
      :empty-msg="$t('inf.benchmark.nofn')"
      :row-class-name="fnRowClassName"
    >
      <let-table-column
        :title="$t('inf.benchmark.interface')"
        prop="interface"
        width="100px"
      ></let-table-column>
      <let-table-column
        :title="$t('inf.benchmark.fnname')"
        prop="name"
        width="120px"
      ></let-table-column>
      <let-table-column
        :title="$t('inf.benchmark.returnType')"
        prop="return"
        width="120px"
      ></let-table-column>
      <let-table-column :title="$t('inf.benchmark.inParam')">
        <template slot-scope="scope">
          <json-view
            ...
            rootKey="view"
            colorScheme="dark"
            :data="JSON.parse(scope.row.inParams)"
          />
        </template>
      </let-table-column>
      <let-table-column :title="$t('inf.benchmark.outParam')">
        <template slot-scope="scope">
          <json-view
            ...
            rootKey="view"
            colorScheme="dark"
            :data="JSON.parse(scope.row.outParams)"
          />
        </template>
      </let-table-column>
      <let-table-column :title="$t('operate.operates')" width="80px">
        <template slot-scope="scope">
          <let-table-operation @click="queryCase(scope.row)">{{
            $t("inf.benchmark.case")
          }}</let-table-operation>
        </template>
      </let-table-column>
    </let-table>
      ...
      <h4>
        {{
          currentFn.interface +
            ...
            currentFn.name +
            " " +
            this.$t("inf.benchmark.caselist")
        }}
        <i
          class="el-icon-refresh-right"
          style="cursor: pointer"
          @click="getBenchmarkCaseList()"
        ></i>
        ...
        <el-button
          ...
          style="float:right"
          class="add-btn"
          @click="initCaseContentForm"
          >{{ $t("inf.benchmark.addCase") }}</el-button
        >
      </h4>
      <br />

      <let-table :data="bmCaseList" :empty-msg="$t('inf.benchmark.nocase')">
        <let-table-column
          :title="$t('inf.benchmark.des')"
          prop="des"
        ></let-table-column>
        <let-table-column
        ...
        <let-table-column :title="$t('operate.operates')" width="240px">
          <template slot-scope="scope">
            ...
            <let-table-operation
              @click="initCaseConfigForm(scope.row, 'test')"
              >{{ $t("inf.benchmark.testCase") }}</let-table-operation
            >
            <let-table-operation
              @click="initCaseConfigForm(scope.row, 'start')"
              >{{ $t("inf.benchmark.runCase") }}</let-table-operation
            >
            <let-table-operation @click="queryResult(scope.row)">{{
              $t("inf.benchmark.viewResult")
            }}</let-table-operation>
            <let-table-operation @click="deleteBmCase(scope.row)">{{
              $t("inf.benchmark.deleteCase")
            }}</let-table-operation>
          </template>
        </let-table-column>
      </let-table>
      ...
      <let-form itemWidth="100%">
        ...
        <let-form-item :label="$t('inf.benchmark.inParam')">
          <json-view
            :maxDepth="0"
            rootKey="view"
            :data="JSON.parse(currentFn.inParams || '{}')"
          />
        </let-form-item>
        <let-form-item :label="$t('inf.benchmark.inputValue')">
          <let-input
            ...
            :rows="10"
            type="textarea"
            v-model="caseModel.in_values"
          ></let-input>
          <a
            target="_blank"
            href="https://doc.tarsyun.com/#/benchmark/tars-guide.md"
          >
            <let-table-operation>{{
              $t("inf.benchmark.inputValueDes")
            }}</let-table-operation>
          </a>
        </let-form-item>
      </let-form>
      ...
      <let-form itemWidth="100%" class="start-bm">
        <let-form-item label="servant">
          <let-input
            disabled
            size="small"
            v-model="caseModel.servant"
          ></let-input>
        </let-form-item>
        <let-form-item :label="$t('inf.benchmark.endpoints')">
          <let-checkbox
            v-for="(value, name) in endpoints"
            v-model="endpoints[name]"
            :key="name"
            class="checkbox_endpoint_item"
            >{{ name }}</let-checkbox
          >
          <let-input
            size="small"
            type="textarea"
            :placeholder="$t('inf.benchmark.endpointsTip')"
            v-model="caseModel.endpoints"
          ></let-input>
        </let-form-item>
        <let-form-group inline label-position="top">
          <let-form-item
            v-if="upsertCaseConfigModalName == 'start'"
            :label="$t('inf.benchmark.links')"
            :size="5"
          >
            <let-input
              type="number"
              size="small"
              v-model="caseModel.links"
              required
            ></let-input>
          </let-form-item>
          <let-form-item
            v-if="upsertCaseConfigModalName == 'start'"
            :label="$t('inf.benchmark.speed')"
            :size="5"
          >
            <let-input
              type="number"
              size="small"
              v-model="caseModel.speed"
              required
            ></let-input>
          </let-form-item>
          <let-form-item
            v-if="upsertCaseConfigModalName == 'start'"
            :label="$t('inf.benchmark.duration')"
            :size="5"
          >
            <let-input
              type="number"
              size="small"
              v-model="caseModel.duration"
              required
            ></let-input>
          </let-form-item>
          <let-form-item
            v-if="testResultModal"
            :label="$t('inf.benchmark.testResult')"
            :size="5"
          >
          ...
          <let-form-item :size="5">
            <let-button
              v-if="upsertCaseConfigModalName == 'test'"
              @click="execBenchmark('test')"
              theme="primary"
              class="start_bm"
              >{{ $t("inf.benchmark.infTest") }}</let-button
            >
            <let-button
              v-if="upsertCaseConfigModalName == 'start'"
              @click="upsertCaseConfig"
              theme="primary"
              class="start_bm"
              >{{ $t("inf.benchmark.startBenchmark") }}</let-button
            >
            <let-button
              @click="upsertCaseConfigModal = false"
              class="cancel_bm"
              >{{ $t("inf.benchmark.cancelTest") }}</let-button
            >
          </let-form-item>
        </let-form-group>
      </let-form>
      ...
      <wrapper>
        <div class="result_op">
          <let-button
            ...
            @click="initCaseConfigForm(currentCase, 'start')"
            size="small"
            theme="success"
            class="bmop-btn"
            >{{ $t("inf.benchmark.startBenchmark") }}</let-button
          ...
          <let-button
            ...
            @click="execBenchmark('stop')"
            size="small"
            theme="danger"
            class="bmop-btn"
            >{{ $t("inf.benchmark.stopBenchmark") }}</let-button
          >
          <let-button
            size="small"
            theme="sub-primary"
            class="bmop-btn"
            @click="getResultById(result.id)"
            >{{ $t("inf.benchmark.refresh")
        ...
        <div class="result_select">
          <let-radio v-model="resultMode" label="detail">{{
            $t("inf.benchmark.detail")
          }}</let-radio>
          <let-radio v-model="resultMode" label="chart">{{
            $t("inf.benchmark.stat")
          }}</let-radio>
        </div>
        <let-table
          v-if="resultMode == 'detail'"
          :data="result.results"
          empty-msg="data is empty"
          style="height:500px;overflow:auto"
        >
          <let-table-column
            :title="$t('inf.benchmark.time')"
            prop="time_stamp"
            width="150px"
            align="center"
          >
          ...
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.total')"
            prop="total_request"
            width="70px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.success')"
            prop="succ_request"
            width="70px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.fail')"
            prop="fail_request"
            width="70px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.successRate')"
            width="80px"
            align="center"
          >
            <template slot-scope="{ row }">
          ...
          <let-table-column
            :title="$t('inf.benchmark.maxCost')"
            prop="max_time"
            width="80px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.minCost')"
            prop="min_time"
            width="80px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.avgCost')"
            width="80px"
            align="center"
          >
            <template slot-scope="{ row }">
              <span>{{
                row.total_request == 0
                  ? 0
                  : (row.total_time / row.total_request).toFixed(2)
              }}</span>
            </template>
          </let-table-column>
          ...
          <let-table-column
            :title="$t('inf.benchmark.sendBytes')"
            prop="send_bytes"
            width="80px"
            align="center"
          ></let-table-column>
          <let-table-column
            :title="$t('inf.benchmark.recvBytes')"
            prop="recv_bytes"
            width="80px"
            align="center"
          ></let-table-column>
        ...
        <div v-if="resultMode == 'chart'" class="return_chart">
          <ve-pie
            :legend="returnChartOptions.legend"
            :title="returnChartOptions.title"
            :data="returnChartOptions"
            width="500px"
          ></ve-pie>
          <ve-pie
            :legend="costChartOptions.legend"
            :title="costChartOptions.title"
            :data="costChartOptions"
            width="500px"
          ></ve-pie>
        </div>
      </wrapper>
...
const CASE_CONTENT = { id: null, des: "", in_values: "" };
...
export default {
  ...
  methods: {
    ...
    initCaseConfigForm(row, mode) {
      ...
      this.caseModel.endpoints = this.caseModel.endpoints || "";
    ...
    upsertCaseContent() {
      ...
      let caseInfo = {};
      if (this.caseModel.id) caseInfo.id = this.caseModel.id;
    ...
    upsertCaseConfig() {
      ...
      if (
        !caseInfo.endpoints ||
        !this.caseModel.links ||
        !this.caseModel.speed ||
        !this.caseModel.duration
      ) {
    ...
    deleteBmCase(row) {
      this.$confirm(
        this.$t("inf.benchmark.deleteCaseConfirm"),
        this.$t("common.alert")
      ).then(() => {
        const loading = this.$Loading.show();
        // let caseInfo = {};
    ...
    objChange() {
      ...
      this.$ajax
        ...
        .then((data) => {
          ...
          data.forEach((item) => {
            this.endpoints[
              `${item.istcp ? "tcp" : "udp"} -h ${item.host} -p ${
                item.port
              } -t ${item.timeout}`
            ] = false;
          });
  ...
  computed: {
    ...
    costChartOptions() {
      return this.pieFromResult(
        this.$t("inf.benchmark.costStat"),
        "返回值",
        "数量",
        "cost_map"
      );
